...
async function migrateEverything() {
  ...
  console.log('✅ Authenticated successfully! Initializing client with session...');
  ...
  // 1. Seed admin_email in settings first so is_admin() resolves correctly
  console.log('Inserting admin_email setting...');
  const { error: emailSettingError } = await supabase.from('settings').upsert([
    { key: 'admin_email', value: 'sweeto@sweetohub.com' },
    { key: 'admin_key', value: 'chibuike@256' }
  ], { onConflict: 'key' });

  if (emailSettingError) {
    console.error('❌ Failed to seed admin settings:', emailSettingError.message);
    return;
  }
  ...
  // 4. Settings (All other settings)
  try {
    const settings = db.prepare("SELECT * FROM settings WHERE key NOT IN ('admin_email', 'admin_key')").all();
    if (settings.length > 0) {
      ...
      else console.log(`✅ Migrated ${settings.length} additional settings.`);
    }
  } catch (e) {
    ...
    if (products.length > 0) {
      const mapped = products.map(p => {
        ...
        return {
          ...
          bought_price: p.bought_price || null,
          stock: p.stock || 0,
  ...
  console.log('🎉 Full Authenticated Migration Complete!');
}
